<?php
    ...
    class fvQuery{
        ...
        public function execute(){
            switch( $this->_type ){
                ...
                case self::STATEMENT_DELETE:
                    fvSite::$pdo->delete(   $this->_rootManager->getTableName(),
                                            $this->getWhere(),
                                            $this->_params);
                    break;
        ...
        private function _getModifiers(){
            ...
            foreach ($this->_managers as $alias => $manager) {
                /** @var $manager fvRootManager */
                foreach ($modifiers as $iEntity => $modificators) {
                    if ($manager->getRootObj()->isImplements($iEntity)) {
