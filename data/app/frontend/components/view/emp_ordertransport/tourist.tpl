<form class="form-inline form-service clearfix" action="/orders/addvehicle" id="vehicle-{$this->entity->getPk()}">
    ...
    <div class="col-lg-3 form-group">
        <select class="form-control vehicle-list" data-room="#seats-{$this->entity->getPk()}" name="data[vehicleId]">
    ...
    <div class="col-lg-3 form-group">
        <select class="form-control" id="seats-{$this->entity->getPk()}" name="data[place]">
            {foreach from=$this->entity->getPlaceList() item=place key=key}
                <option value="{$key}" {if $this->entity->place eq $key}selected{/if} >{$place} - {if $place->isBusy()}{else}Свободно{/if}</option>
            {/foreach}
        </select>
    </div>

    <div class="col-lg-3 form-group">
        <input class="form-control" value="{$this->entity->getTransportComment()}" type="text" name="data[comment]" id="service-comment-{$this->entity->getPk()}" />
    </div>
