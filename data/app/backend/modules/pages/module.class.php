...
class PagesModule extends fvModule {
    ...
    function __construct() {
        ...
        parent::__construct(fvSite::$fvConfig->get("modules.{$this->moduleName}.smarty.template"), fvSite::$fvConfig->get("modules.{$this->moduleName}.smarty.compile"), fvSite::$Layoult);
    }

    function showIndex() {
        $pages = Page::getManager()->getPageTree();
        $this->pageTree = $pages;


        $request = fvRequest::getInstance();
        if (!$Page = Page::getManager()->getByPk($request->getRequestParameter('id'))) {
            $Page = new Page();
        }

        $this->__assign(array(
            'Page' => $Page,
            'PageManager' => Page::getManager(),
        ));
    ...
    function showGetpagecontent() {
        ...
        if ($dom->loadXML($_xmlContent)) {
            ...
            if ($nodeId = fvRequest::getInstance()->getRequestParameter('_nodeId')) {
                ...
                $currentPartName = fvRequest::getInstance()->getRequestParameter('_partName', null, $this->appConfig->get("template.{$this->getRequest()->getRequestParameter("_template", "string", "main")}.default_content_part"));
            ...
            $pageParts = $this->appConfig->get( "template.{$this->getRequest()->getRequestParameter("_template", "string", "main")}.content_parts" );
//            var_dump( $pageParts );
    ...
    function showGetmoduleparams() {
        ...
        foreach ($this->appConfig->get("modules") as $moduleName => $module) {
            $shows = $this->appConfig->get("modules.{$moduleName}.shows");
            if( !$module['system'] && !empty( $shows ) )
